...
### 问题二  
#### (1) get_pte() 中有两段形式类似的代码，解释其在 sv32、sv39、sv48 下为何相像。

**回答：**  
sv32、sv39、sv48 虽然级数不同（sv32 二级，sv39、sv48 三级 / 四级），但每一级页表处理方式完全相同：  
- 都需要根据线性地址某些位取出对应的页表索引；  
- 都需要根据索引从当前页表取 PTE；  
- 若 `create = 1` 且不存在对应页表，则分配新页表并填入 PTE；  
- 然后进入下一层页表继续查找。

因此，无论多少级页表，“每一级的处理步骤” 都一致，只是“循环次数”不同——这导致 get_pte() 中会出现两段（或循环中多段） **结构相似** 的代码。

---

#### (2) get_pte() 将查找与分配合并在同一函数中是否合适？是否应拆分？

**回答：**  
把查找与分配写在同一个函数中可以简化接口，让调用者只需一次调用即可获得最终 PTE。但这也导致函数职责不够单一，不便于只查找而不想创建新页表时使用。  
因此从软件工程角度看，**拆分为 get_pte()（仅查找）和 create_pte()（必要时创建）会更清晰**；但在教学实验中保持合并实现也完全可行，利于减少代码复杂度。
