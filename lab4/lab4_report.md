...
**回答：**  
`local_intr_save(intr_flag)` 会读取当前的中断使能状态（通常由 sstatus/SIE 位表示），把这一状态保存到 `intr_flag` 中，并同时关闭本地中断（清除 SIE）。  
`local_intr_restore(intr_flag)` 则根据 `intr_flag` 的值恢复之前的中断状态：如果之前中断是开启的，则重新设置 SIE；如果之前是关闭的，则保持关闭。  
因此，这两个语句共同实现了：**保存当前中断状态 → 强制关中断执行临界区 → 恢复到执行临界区之前的中断状态**。

---
### 问题二（基于给出的 get_pte() 代码）

#### (1) 为什么 get_pte() 中出现两段几乎相同的代码？结合 sv32、sv39、sv48 的异同解释。

在给出的实现中：

- 第一段代码处理 `pdep1` —— 页目录的“高一级”页表；
- 第二段代码处理 `pdep0` —— 页目录的“下一层”页表。

两段代码逻辑几乎一致：

1. 判断该层页表项是否有效（检查 PTE_V）
2. 若无效且允许 `create`，则分配新页表页
3. 清零该页
4. 写入新的页表项，使其指向下一层页表

这两段结构相似，是因为：

- **sv32、sv39、sv48 的页表层数不同，但 “每一层的处理方式是完全相同的”。**
- sv32：2 级  
  → 对应代码中的 pdep1 和 pdep0（正好两段）
- sv39：3 级  
  → 实质上应再多一段，但本实验为了简化结构，将 sv39/48 折叠为“固定两级”处理
- sv48：4 级  
  → 也同理，每一级的动作都是相同的

因此，虽然不同模式层数不同，但：

> **每一级的逻辑：检查 → 分配 → 初始化 → 进入下一级** 是完全一致的，所以代码自然呈现重复结构。

---

#### (2) get_pte() 将查找和分配放在一个函数里是否合适？是否需要拆分？

从这段代码可以看到：

- 函数同时完成：
  - **查找** 某线性地址对应的页表项
  - **必要时分配** 中间页表（当 `create = 1`）

这样写的优点：

- 调用者无需关心页表层级，不需要写额外代码初始化缺失的页表
- 代码短、接口简单，非常适合教学实验环境

缺点：

- 查找逻辑与分配逻辑混在一起，**职责不单一**
- 若仅想“查找但不想创建”，必须依赖参数 `create`
- 对于需要精细控制页表结构的情况不够灵活
...
- `pte_t *lookup_pte(pgdir, la)` —— 只查找
- `pte_t *ensure_pte(pgdir, la)` —— 必要时分配中间页表

但对于本课程实验，保持当前写法完全没问题。
